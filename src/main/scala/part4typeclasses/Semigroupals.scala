...
object Semigroupals {
  ...
  val aTupledOption = optionSemigroupal.product(Some(123), Some("a string")) // Don't have to unwrap / wrap items
  ...
  val aTupledList = Semigroupal[List].product(List(1, 2), List("a", "b")) // cartesian product (monad rather than a zip)
